    ...
    public function idiomasAction(Request $request)
    {
        ...
        if ($idiomas) {
            foreach($idiomas as $idioma) {
                ...
                //subtitulos
                $subtitulos = array();
                if($idioma->getSubtitulosPeliculas()){
                  foreach ($idioma->getSubtitulosPeliculas() as $subtitulo) {
                    $subtitulos[] = array(
                      'id' =>  $subtitulo->getId(),
                      'nombre' => $subtitulo->getNombre(),
                      'descripcion' => $subtitulo->getResumen(),
                      'urlTrailer' => $subtitulo->getUrlTrailer(),
                     );
                  }
                }else{
                  $subtitulos[] =  array();
                ...
                //audios
                $audios = array();
                if($idioma->getAudioPeliculas()){
                  foreach ($idioma->getAudioPeliculas() as $audio) {
                    $audios[] = array(
                      'id' =>  $audio->getId(),
                      'nombre' => $audio->getNombre(),
                      'descripcion' => $audio->getResumen(),
                      'urlTrailer' => $audio->getUrlTrailer(),
                     );
                  }
                }else{
                  $audios[] =  array();
                ...
                $output[] = array(
                    ...
                    'peliculas subtitulos'  => $subtitulos,
                    'peliculas audios'      => $audios,
                );
            }
    ...
    public function idiomaAction(Request $request, $id)
    {
        ...
        if ($idioma) {
          ...
          $output[] = array(
            ...
            return new View('Idioma no encontrado', Response::HTTP_NOT_FOUND);
        }
    }
